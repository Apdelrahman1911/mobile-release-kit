"""One fixed main-thread diagnostics request, then held-pipe EOF cancellation.

No multi-frame edit protocol, reconnect, generic callback or competing reader.
The child-local six-second endpoint cannot renew the earlier native T/W/H.
"""
...
from ._desktop_environment_protocol import (EnvironmentRequest, ProtocolError, REQUEST_LIMIT,
...
class EnvironmentInput:
    ...
    def poll(self, guard: DefaultCancellation) -> None:
        ...
        try:
            ...
            if (value.st_dev, value.st_ino, value.st_mode) != self.identity:
                ...
                raise ProtocolError("Original diagnostics input changed")
    ...
    def request(self) -> EnvironmentRequest:
        self._owner()
    ...
    def close(self) -> None:
        ...
        try:
            ...
            self.closed = True
            if self.guard is not None:
                self.guard._remove_environment_source(self)
        except BaseException as error:
            if self.guard is not None:
                self.guard._abort(error)
            raise
