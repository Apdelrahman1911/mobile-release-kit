"""Dedicated original offline preflight child; ordinary owned commands may descend.
...
class _Engine:
    def __init__(self, started: float) -> None:
        ...
        self.guard = DefaultCancellation(ProcessCleanupError, "Saved offline preflight custody did not settle")
        self.input = PreflightInput(started)
        self.output, self.error_output = _Output(1), _Output(2)
        self.request: PreflightRequest | None = None
        self.service: OfflinePreflightRun | None = None
    ...
    def retain_unknown(self) -> None:
        if (self.guard.lifetime_ledger.fatal or not self.input.closed
                or self.service is not None and not self.service.budget.closed
