...
pub(crate) mod github_tls {
    ...
    // Same retained original peer owner; the installed profile supplies a
    // separate admitted launch, never this development namespace selection.
    use crate::supervisor::github_tls_peer_owner::{Peer, PeerControl, ready, guarded};
    impl Peer {
        fn begin(&mut self, common: &Common, name: &'static str) -> Check<()> {
            self.begin_paths(common.manifest.python.clone(), common.manifest.role("peer")?.to_path_buf(),
                common.peer_env.clone(), name)
        }
    }
    ...
    mod deadline {
        ...
        pub(super) use crate::supervisor::github_tls_peer_owner::Arrivals;
        #[derive(Clone, Deserialize)]
        #[serde(rename_all = "camelCase", deny_unknown_fields)]
        struct Progress {
            schema_version: u32, scope: String, #[serde(rename = "case")] case_name: String, state: String,
