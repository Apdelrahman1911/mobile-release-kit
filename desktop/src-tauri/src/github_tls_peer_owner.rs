    ...
    impl InstalledPeer {
        ...
        pub(crate) fn prepare(&mut self, root:PathBuf, end:Instant)->Check<()> {
            ...
            for (kind,role) in [("net","NETNS"),("mnt","MNTNS"),("user","USERNS"),("pid","PIDNS")] {
                let current=fs::read_link(format!("/proc/self/ns/{kind}")).map_err(|_|"installed_peer_domain")?;
                let initial=fs::read_link(format!("/proc/1/ns/{kind}")).map_err(|_|"installed_peer_domain")?;
                require(current==initial,"installed_peer_initial_domain")?;
                let value=current.to_str().ok_or("installed_peer_domain")?.to_owned();
                environment.insert(format!("MRK_TLS_PARENT_{role}"),value.clone());environment.insert(format!("MRK_TLS_{role}"),value);
            }
