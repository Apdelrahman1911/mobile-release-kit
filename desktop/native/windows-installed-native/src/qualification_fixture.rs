...
pub(super) const NORMAL_UI_PROFILE: &str = "windows-normal-project-ui-v1";
pub(super) const NORMAL_UI_DISPATCH: &str = "windows-normal-project-ui";
pub(super) const NORMAL_UI_STAGE: &str = "qualification_fixture::hosted_stage_normal_ui_runtime_input";
pub(super) const NORMAL_UI_OBSERVE: &str = "qualification_fixture::hosted_observe_normal_ui_published_version";
pub(super) const NORMAL_UI_PRECHECK_FILE: &str = "normal-ui-setup-precheck.private.txt";
pub(super) const NORMAL_UI_STAGE_FILE: &str = "normal-ui-setup-stage-result.private.txt";
pub(super) const NORMAL_UI_OBSERVATION_FILE: &str = "normal-ui-setup-publication-observation.private.txt";
pub(super) const NORMAL_UI_PRECHECK_HEADER: &str = "MRK_WINDOWS_NORMAL_UI_SETUP_PRECHECK_V1";
pub(super) const NORMAL_UI_OBSERVATION_HEADER: &str = "MRK_WINDOWS_NORMAL_UI_SETUP_OBSERVATION_V1";
pub(super) const NORMAL_UI_SETUP_EXIT_HEADER: &str = "MRK_WINDOWS_NORMAL_UI_SETUP_EXIT_V1";
...
pub(super) const NORMAL_UI_SETUP_PROOFS: [(&str, &str, usize); 3] = [
    ("stage", NORMAL_UI_STAGE_FILE, OWNER_LIMIT), ("stageExit", "normal-ui-setup-stage-exit.private.txt", LIMIT),
    ("helperSuccessExit", "normal-ui-setup-publisher-exit.private.txt", LIMIT),
];
...
// Runtime setup has no app/headless claim. Its owner is a DISTINCT anchored
// runtime-publication-only artifact, never the retained cheap desktop-ui owner.
pub(super) const NORMAL_UI_PRECHECK_FIELDS: [&str; 40] = [
    "profile", "sourceSha", "sourceTree", "runId", "attempt", "sourceInventorySha256",
    "prerequisiteRequestSha256", "prerequisiteFinalizerSha256",
    "ownerArtifact", "ownerArtifactBytes", "ownerArtifactSha256", "ownerArtifactIdentity",
    "ownerCompileMessagesBytes", "ownerCompileMessagesSha256", "ownerCompileArgvSha256",
    "stageTest", "stageCommandSha256", "observerTest", "observerCommandSha256", "standaloneFeatures",
    ...
    "helperCommandSha256", "helperNativeFeatures", "manifestSha256", "protocolSha256",
    "inventorySha256", "coreSha256", "payloadFiles", "payloadBytes", "preparedReceiptBytes",
    "preparedReceiptSha256", "rosterBytes", "rosterSha256", "setupContract",
];

// Closed role table shared only by the three explicit setup/observation lanes.
// It selects existing custody/mutation bodies; it cannot dispatch a process or
// choose a schema supplied by the caller. Fullwalk fixture/retire stays separate.
#[derive(Clone, Copy)]
struct SetupRoute {
    profile: &'static str, stage: &'static str, observe: &'static str,
    precheck_file: &'static str, observation_header: &'static str, exit_header: &'static str,
    preflight_outcome: &'static str, proofs: &'static [(&'static str, &'static str, usize)],
}
fn setup_route(profile: &str) -> Result<SetupRoute> {
    match profile {
        PRODUCTION_PROFILE => Ok(SetupRoute { profile: PRODUCTION_PROFILE, stage: STAGE_INPUT, observe: OBSERVE_BEFORE,
            precheck_file: PRECHECK_FILE, observation_header: OBSERVATION_HEADER, exit_header: PRODUCER_EXIT_HEADER,
            preflight_outcome: "MRK_WINDOWS_ORDINARY_PREFLIGHT_STEP_OUTCOME", proofs: &PRODUCER_PROOFS }),
        PASSIVE_PROFILE => Ok(SetupRoute { profile: PASSIVE_PROFILE, stage: PASSIVE_STAGE, observe: PASSIVE_OBSERVE,
            precheck_file: PASSIVE_PRECHECK_FILE, observation_header: PASSIVE_OBSERVATION_HEADER, exit_header: PASSIVE_SETUP_EXIT_HEADER,
            preflight_outcome: "MRK_WINDOWS_PASSIVE_PREFLIGHT_STEP_OUTCOME", proofs: &PASSIVE_SETUP_PROOFS }),
        NORMAL_UI_PROFILE => Ok(SetupRoute { profile: NORMAL_UI_PROFILE, stage: NORMAL_UI_STAGE, observe: NORMAL_UI_OBSERVE,
            precheck_file: NORMAL_UI_PRECHECK_FILE, observation_header: NORMAL_UI_OBSERVATION_HEADER, exit_header: NORMAL_UI_SETUP_EXIT_HEADER,
            preflight_outcome: "MRK_WINDOWS_UI_SETUP_PREFLIGHT_STEP_OUTCOME", proofs: &NORMAL_UI_SETUP_PROOFS }),
        _ => Err(Error::Unsafe),
    }
}
fn setup_test(test: &str) -> Result<SetupRoute> {
    setup_route(match test {
        STAGE_INPUT | OBSERVE_BEFORE | OBSERVE_AFTER => PRODUCTION_PROFILE,
        PASSIVE_STAGE | PASSIVE_OBSERVE => PASSIVE_PROFILE,
        NORMAL_UI_STAGE | NORMAL_UI_OBSERVE => NORMAL_UI_PROFILE,
        _ => return Err(Error::Unsafe),
    })
}
impl SetupRoute {
    fn proof_count(self, count: usize) -> Result<()> {
        need(count == 3 || self.profile == PRODUCTION_PROFILE && count == 6)
    }
    fn observer(self, after: bool) -> Result<&'static str> {
        need(!after || self.profile == PRODUCTION_PROFILE)?;
        Ok(if after { OBSERVE_AFTER } else { self.observe })
    }
    fn producer_observed(self, test: &str) -> bool {
        test == if self.profile == PRODUCTION_PROFILE { OBSERVE_AFTER } else { self.observe }
    }
}
...
pub(super) fn precheck(raw: &[u8], root: &Path) -> Result<Wire> {
    if active_profile()? == NORMAL_UI_PROFILE { return normal_ui_setup_precheck(raw, root); }
...
pub(super) fn normal_ui_setup_features(enabled: [bool; 5]) -> Result<()> {
    // runtime-publication, qualification-result, desktop-ui, desktop-ui-dialogs,
    // windows-installed-observation. No UI crate/owner or result-writer feature.
    need(enabled == [true, false, false, false, false])
}
pub(super) fn normal_ui_setup_values(value: &Wire, manifest: &str, protocol: &str) -> Result<()> {
    need(is_hex(manifest, 64) && is_hex(protocol, 64))?;
    for (key, expected) in [
        ("profile", NORMAL_UI_PROFILE), ("stageTest", NORMAL_UI_STAGE), ("observerTest", NORMAL_UI_OBSERVE),
        ("standaloneFeatures", "runtime-publication"), ("helperNativeFeatures", "runtime-publication"),
        ("setupContract", "fixed-normal-ui-fresh-runtime-only-v1"),
        ("manifestSha256", manifest), ("protocolSha256", protocol),
    ] { value.equal(key, expected)?; }
    for role in ["owner", "helper"] {
        original_epoch(value.get(&format!("{role}ArtifactIdentity"))?, value.get(&format!("{role}ArtifactIdentity"))?)?;
        need(value.number(&format!("{role}ArtifactBytes"), PAYLOAD_LIMIT as u64)? > 0
            && value.number(&format!("{role}CompileMessagesBytes"), 16 << 20)? > 0)?;
    }
    need(value.number("payloadFiles", 2047)? == 46 && value.number("payloadBytes", 1 << 30)? > 0
        && value.number("preparedReceiptBytes", 4096)? > 0 && value.number("rosterBytes", TEXT_LIMIT as u64)? > 0)
}
pub(super) fn normal_ui_setup_probe_gate(value: &Wire, available: &str, finalized: &str,
    request_sha: &str, finalizer_sha: &str) -> Result<()> {
    need(available == "true" && finalized == "success" && is_hex(request_sha, 64) && is_hex(finalizer_sha, 64))?;
    value.equal("prerequisiteRequestSha256", request_sha)?;
    value.equal("prerequisiteFinalizerSha256", finalizer_sha)
}
fn normal_ui_setup_precheck(raw: &[u8], root: &Path) -> Result<Wire> {
    normal_ui_setup_features([cfg!(feature = "runtime-publication"), cfg!(feature = "qualification-result"),
        cfg!(feature = "desktop-ui"), cfg!(feature = "desktop-ui-dialogs"), cfg!(feature = "windows-installed-observation")])?;
    let value = Wire::parse(raw, NORMAL_UI_PRECHECK_HEADER, &NORMAL_UI_PRECHECK_FIELDS, TEXT_LIMIT)?;
    value.binding()?;
    normal_ui_setup_values(&value, option_env!("MRK_BUNDLED_RUNTIME_MANIFEST_SHA256").ok_or(Error::State)?,
        option_env!("MRK_BUNDLED_PROTOCOL_SHA256").ok_or(Error::State)?)?;
    let get = |name: &str| std::env::var(name).map_err(|_| Error::State);
    normal_ui_setup_probe_gate(&value, &get("MRK_WINDOWS_UI_PREREQUISITES_AVAILABLE")?,
        &get("MRK_WINDOWS_UI_PREREQUISITE_FINALIZE_STEP_OUTCOME")?,
        &get("MRK_WINDOWS_UI_PREREQUISITE_REQUEST_SHA256")?, &get("MRK_WINDOWS_UI_PREREQUISITE_FINALIZER_SHA256")?)?;
    need(get("MRK_WINDOWS_UI_SETUP_PRECHECK_SHA256")? == digest(raw)?)?;
    // Caller preflight has joined the positive probe's actual pre/post ACL
    // identity and closed finalizer, then the DISTINCT setup compiler original.
    // This runtime-only precheck never authorizes another ordinary account.
    let owner = fixed_path(value.get("ownerArtifact")?)?;
    need(owner.parent() == Some(fixed_directories(root)[3].1.as_path()))?;
    let leaf = owner.file_name().and_then(|name| name.to_str()).ok_or(Error::Unsafe)?;
    need(leaf.strip_prefix("mrk_windows_installed_native-").and_then(|name| name.strip_suffix(".exe"))
        .is_some_and(|name| is_hex(name, 16)))?;
    for (key, test) in [("stageCommandSha256", NORMAL_UI_STAGE), ("observerCommandSha256", NORMAL_UI_OBSERVE)] {
        value.equal(key, &command_sha(value.get("ownerArtifact")?, test)?)?;
    }
    need(fixed_path(value.get("helperArtifact")?)? == root.join("mrk-windows-runtime-publish.exe"))?;
    value.equal("helperCommandSha256", &digest(&format!("\"{}\"", value.get("helperArtifact")?)
        .encode_utf16().flat_map(u16::to_le_bytes).collect::<Vec<_>>())?)?;
    Ok(value)
}
...
pub(super) fn observation(raw:&[u8],items:&[Payload],test:&str) -> Result<(Wire,Vec<ObservedObject>,Vec<ProducerProof>)> {
    let route=setup_test(test)?;
    need(items.len()==47
        && !raw.is_empty() && raw.len()<=OWNER_LIMIT && raw.is_ascii() && raw.ends_with(b"\n") && !raw.contains(&b'\r'))?;
    let stage=test==route.stage;let proof_count=if stage {0}else if test==OBSERVE_AFTER {6}else{3};
    ...
    let record=Wire::parse(header.as_bytes(),route.observation_header,&OBSERVATION_FIELDS,OWNER_LIMIT)?;
    record.binding()?;record.equal("profile",route.profile)?;record.equal("observerTest",test)?;
    for (key,value) in [("payloadFiles","46"),("sourceReaders",if stage {"47"}else{"0"}),
        ...
        ("actualFixedProducerObserved",if route.producer_observed(test) {"true"}else{"false"}),
    ...
    for (line,(role,_,limit)) in lines[prefix..prefix+proof_count].iter().zip(route.proofs) {
...
pub(super) fn producer_exit(raw:&[u8],pre:&Wire,pre_raw:&[u8],role:&str) -> Result<()> {
    let route=setup_route(active_profile()?)?;
    let (artifact,test,code)=match role {
        "stage"=>("owner",Some(route.stage),"0"),"helperSuccess"=>("helper",None,"0"),
        "observe" if route.profile!=PRODUCTION_PROFILE=>("owner",Some(route.observe),"0"),
        "before" if route.profile==PRODUCTION_PROFILE=>("owner",Some(OBSERVE_BEFORE),"0"),
        "after" if route.profile==PRODUCTION_PROFILE=>("owner",Some(OBSERVE_AFTER),"0"),
        "helperOccupied" if route.profile==PRODUCTION_PROFILE=>("helper",None,"2"),_=>return Err(Error::Unsafe),
    };
    let record=Wire::parse(raw,route.exit_header,&PRODUCER_EXIT_FIELDS,LIMIT)?;
    record.binding()?;record.equal("profile",route.profile)?;record.equal("role",role)?;
...
fn production_outcomes(count:usize) -> Result<()> {
    let route=setup_route(active_profile()?)?;
    if route.profile!=PRODUCTION_PROFILE {
        need(count==3)?;
        let prefix=if route.profile==PASSIVE_PROFILE {"MRK_WINDOWS_PASSIVE"}else{"MRK_WINDOWS_UI_SETUP"};
        for role in ["STAGE","PUBLISH"] {
            outcome(&format!("{prefix}_{role}_STEP_OUTCOME"))?;
            outcome(&format!("{prefix}_{role}_FINALIZE_STEP_OUTCOME"))?;
        }
        return Ok(());
    }
...
fn checked_producer_chain(pre:&Wire,pre_raw:&[u8],roster_raw:&[u8],items:&[Payload],proof_raw:&[Vec<u8>])
    ...
    let route=setup_route(active_profile()?)?;
    route.proof_count(proof_raw.len())?;
    let (_,stage,stage_proofs)=bound_observation(&proof_raw[0],route.stage,pre,pre_raw,roster_raw,items)?;
...
impl Fixture {
    fn publication_inputs(&mut self) -> Result<(Wire,Vec<Payload>,Vec<u8>,Vec<u8>)> {
        ...
        let precheck_name=if profile==PROFILE {PRECHECK_FILE}else{setup_route(profile)?.precheck_file};
    ...
    fn stage_source(&mut self) -> Result<(Wire,Vec<ObservedObject>,Vec<ProducerProof>)> {
        let route=setup_route(active_profile()?)?;
        outcome(route.preflight_outcome)?;
...
fn producer_observation_run(test:&str,start:Instant) -> Result<()> {
    let route=setup_test(test)?;
    if route.profile==NORMAL_UI_PROFILE {
        normal_ui_setup_features([cfg!(feature = "runtime-publication"), cfg!(feature = "qualification-result"),
            cfg!(feature = "desktop-ui"), cfg!(feature = "desktop-ui-dialogs"), cfg!(feature = "windows-installed-observation")])?;
    }else {
        need(if route.profile==PASSIVE_PROFILE {!cfg!(feature="runtime-publication") && !cfg!(feature="qualification-result")}
            else {super::ordinary_owner::FULLWALK_PREREQUISITES_REVIEWED})?;
    }
    profile()?;
    need(active_profile()?==route.profile)?;
    if route.profile==NORMAL_UI_PROFILE {outcome(route.preflight_outcome)?;}
    let root=fixed_path(&std::env::var("MRK_DESKTOP_CI_ROOT").map_err(|_|Error::State)?)?;
    let temp=fixed_path(&std::env::var("RUNNER_TEMP").map_err(|_|Error::State)?)?;
    let run=std::env::var("GITHUB_RUN_ID").map_err(|_|Error::State)?;
    need(decimal(&run) && root==temp.join(format!("mrk-windows-installed-native-{run}-1"))
    ...
    let mut original=Fixture::new(start,root,image)?;
    let observed=std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        if test==route.stage {original.stage_source()}else{original.observe_producer(test==OBSERVE_AFTER)}
    })).unwrap_or(Err(Error::Unknown));
    if matches!(observed,Err(Error::Unknown)) || original.unknown() {
    ...
    record.put("fileOriginals",original.opened);record.put("fileOriginalsClosed",original.closed);
    let mut raw=record.encoded(route.observation_header,&OBSERVATION_FIELDS,OWNER_LIMIT)?;
    ...
    let items=PAYLOAD_NAMES.iter().map(|name| {
        let role=if test==route.stage {format!("source/{name}")}else{(*name).to_owned()};
    ...
    let name=match test {STAGE_INPUT=>STAGE_FILE,OBSERVE_BEFORE=>BEFORE_FILE,OBSERVE_AFTER=>PUBLICATION_FILE,
        PASSIVE_STAGE=>PASSIVE_STAGE_FILE,PASSIVE_OBSERVE=>PASSIVE_OBSERVATION_FILE,
        NORMAL_UI_STAGE=>NORMAL_UI_STAGE_FILE,NORMAL_UI_OBSERVE=>NORMAL_UI_OBSERVATION_FILE,_=>return Err(Error::State)};
...
#[test]
#[ignore = "fresh runtime-only normal UI setup after positively finalized native prerequisite; no GUI/account admission"]
fn hosted_stage_normal_ui_runtime_input() -> Result<()> {
    let start=Instant::now();producer_observation_run(NORMAL_UI_STAGE,start)
}
#[test]
#[ignore = "read-only original103-object normal UI setup after original stage/helper and independent finalizers"]
fn hosted_observe_normal_ui_published_version() -> Result<()> {
    let start=Instant::now();producer_observation_run(NORMAL_UI_OBSERVE,start)
}
