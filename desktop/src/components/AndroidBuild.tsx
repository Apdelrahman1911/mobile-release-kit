...
import { androidBuildCancelHelp, androidBuildHelp, androidBuildInputHelp, androidBuildOutputHelp, androidBuildOwnerReason } from '../androidBuild.ts';
import { ANDROID_BUILD_CORE_STATUSES, androidBuildAvailabilityText, androidBuildFindingText, androidBuildLimitationText, androidBuildReasonText } from '../androidBuildProtocol.ts';
...
export function AndroidBuildResultView({ state, operationProjectName = null }: { state: AndroidBuildState; operationProjectName?: string | null }) {
  ...
  return <section className="offline-report" aria-label="Completed local Android AAB observation">
    ...
    <ol className="offline-findings">{result.findings.map((finding) => <li key={finding.ordinal}><Badge tone={negative(finding.status) ? 'warning' : 'neutral'}>{finding.status}</Badge><span>{androidBuildFindingText[finding.check]}</span></li>)}</ol>
  ...
  return <section className="card offline-preflight" aria-labelledby={label}>
    ...
    {consent && <div className="session-review" role="group" aria-label="Confirm this saved Android-build intent">
      ...
      <label className="offline-ack"><input type="checkbox" checked={consent.acknowledged} onChange={(event) => controller.setAcknowledged(consent.operationId, consent.ownerGeneration, event.target.checked)} />
        <span>I trust this project and authorize one build of these saved Android inputs. I understand project-code effects, that my unsaved draft is not used, and that post-run bytes may be reused/stale, signer is not inspected, and completion is not release approval.</span></label>
    ...
    {op && <div className="session-progress" role="status" aria-live="polite">
      ...
      {op.activity && op.outcome !== 'complete' && <ol className="offline-findings">{op.activity.findings.map((finding) => <li key={finding.ordinal}><Badge tone={negative(finding.status) ? 'warning' : 'neutral'}>{finding.status}</Badge><span>{androidBuildFindingText[finding.check]}</span></li>)}</ol>}
