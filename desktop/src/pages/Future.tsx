...
export function Releases({ info, offlineChecks }: { info: AppInfo | null; offlineChecks: ReactNode }) {
  return <>
    <PageHeading eyebrow="RELEASES" title="One candidate. A traceable journey." description="Review saved offline checks explicitly. Release-candidate creation and protected promotion remain future stages." />
    {offlineChecks}
...
export function Recovery({ info }: { info: AppInfo | null }) {
  return <>
    ...
    <p className="review-caution">Offline-check cancellation cannot undo effects already performed by configured project code. Project admission refusal does not establish a recoverable signing session or distinguish busy ownership from recovery need. Use the original operation/status/recovery procedure; this page cannot inspect, clean or reset it.</p>
