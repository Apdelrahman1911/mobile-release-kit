...
export function Environment({ info, preview, onRetry, loading }: { info: AppInfo | null; preview: boolean; onRetry: () => void; loading: boolean }) {
  ...
  return <>
    ...
    })}</div><p className="save-note">Native configuration saving has a separate owned-session capability in Project settings. These passive capabilities never enable saving, production runtime admission or Windows mutation.</p></section>
