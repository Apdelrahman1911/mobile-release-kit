...
function AssetGuide({ guide, sessionAvailable, onHelp }: { guide: CredentialGuide | null; sessionAvailable: boolean; onHelp: (help: HelpContent) => void }) {
  ...
  return <section className="card asset-guide">
    ...
    <div className="asset-guide-layout">
      ...
      <div className="asset-guide-detail">
        ...
        <ul className="asset-guide-fields">{kind.fields.map((field) => <li key={field.id}>
          ...
          {field.input === 'file' && <p className="asset-guide-file-note"><Icon name="folder" size={16} /><span>{sessionAvailable && (kind.id === 'android-keystore' || kind.id === 'android-firebase') ? 'Use the session importer above for supported JKS or Android JSON files. It preserves your original; this guide itself does not read files.' : 'Native selection of this file type is not available in this build. No manual copying or renaming is needed for this guide.'}</span></p>}
        </li>)}</ul>
    ...
    <details className="asset-guide-explainer"><summary>Understand selection, storage, and assignment</summary><p>{sessionAvailable ? 'The session importer above enables only its explicitly listed formats and in-memory actions. Persistent storage and the other file families remain unavailable.' : 'These controls require native qualification before they become available. This guide does not import, store, assign, replace or delete anything.'}</p><div className="asset-guide-control-grid">{guide.controls.map((control) => <div key={control.id}><div className="inline-heading"><h4>{control.label}</h4><HelpButton content={control} onHelp={onHelp} /></div><p>{control.what}</p></div>)}</div></details>
...
export function Credentials({ catalog, state, controller, project, onHelp }: { catalog: Catalog | null; state: AssetDisplayState; controller: AssetSessionController; project: ProjectSession | null; onHelp: (help: HelpContent) => void }) {
  ...
  return <>
    ...
    <CredentialSession state={state} controller={controller} project={project} guide={catalog?.credentialGuide ?? null} onHelp={onHelp} />
    <AssetGuide guide={catalog?.credentialGuide ?? null} sessionAvailable={state.mode === 'native' && state.status?.capability.available === true && !state.blocked} onHelp={onHelp} />
