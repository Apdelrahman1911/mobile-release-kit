...
impl<T> Task<T> {
    ...
    /// Request cancellation of this exact Tokio task without dropping its join.
    /// This is a request only; it is never a termination receipt.
    #[cfg(feature = "tokio")]
    pub(crate) fn request_tokio_cancel(&mut self) -> bool {
        if let Some(handle) = self.tokio.as_ref() {
            handle.abort();
            true
        } else {
            false
    ...
    /// Consume the actual original join without the legacy Future's panic path.
    /// None means there is no available Tokio handle, NOT that work terminated.
    #[cfg(feature = "tokio")]
    pub(crate) fn poll_tokio_join(
        &mut self,
        cx: &mut Context<'_>,
    ) -> Poll<Option<std::result::Result<T, tokio::task::JoinError>>> {
        let Some(handle) = self.tokio.as_mut() else { return Poll::Ready(None); };
        match Pin::new(handle).poll(cx) {
            Poll::Pending => Poll::Pending,
            Poll::Ready(result) => {
                self.tokio = None; // Only after the actual JoinHandle returned.
