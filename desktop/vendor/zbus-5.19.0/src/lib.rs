...
// These finite private-state checks are compiled only for the explicit Linux
// app test graph, never a shipping/release or unqualified-platform graph.
#[cfg(all(feature = "mrk-owned-test-support", not(all(
    debug_assertions, feature = "tokio", target_os = "linux",
    target_arch = "x86_64", target_env = "gnu"
))))]
compile_error!("mrk-owned-test-support requires the reviewed debug Linux x86_64 GNU Tokio test profile");
