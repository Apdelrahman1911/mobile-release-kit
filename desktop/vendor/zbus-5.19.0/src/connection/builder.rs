...
impl<'a> Builder<'a> {
    ...
    /// The maintained owned route deliberately cannot enroll names, interfaces,
    /// a second executor, peer-credential work or a caller-selected transport.
    /// Authentication and Connection construction use the ordinary SDK code.
    #[cfg(all(unix, feature = "tokio"))]
    pub(super) async fn build_owned(
        ...
        if !matches!(self.target, Some(Target::TokioUnixStream(_)))
            ...
            || self.max_queued != Some(1) || self.method_timeout.is_some()
            || self.auth_mechanism.is_some() || self.guid.is_some() || self.user_id.is_some()
        {
            return Err(Error::Unsupported);
        }
        #[cfg(feature = "p2p")]
        if self.p2p { return Err(Error::Unsupported); }
        #[cfg(feature = "bus-impl")]
        if self.unique_name.is_some() { return Err(Error::Unsupported); }
        ...
        // reader handle before this original future can return or fail later.
        custody.install_connection(connection, socket_read, already_read, already_received_fds)
    }
