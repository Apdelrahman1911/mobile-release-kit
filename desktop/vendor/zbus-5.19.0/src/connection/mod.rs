...
// Maintained, fixed-topology client. Ordinary Connection/MessageStream values
// never escape this owner, so its reader roster remains true after startup.
#[cfg(all(unix, feature = "tokio"))]
mod owned;
#[cfg(all(unix, feature = "tokio"))]
pub use owned::{LocalIoError, LocalSettlement, OwnedConnectionAttempt, ReaderOutcome};
#[cfg(all(unix, feature = "tokio", feature = "mrk-owned-test-support"))]
#[doc(hidden)]
pub use owned::test_support as owned_test_support;
...
impl Connection {
    ...
    pub(crate) fn init_socket_reader(
        ...
        self.inner
            ...
            .set(
                self.socket_reader(
                    socket_read, already_read,
                    #[cfg(unix)]
                    already_received_fds,
                )
                .spawn(&self.inner.executor),
            )
    ...
    fn socket_reader(
        ...
        SocketReader::new(
            socket_read, self.inner.msg_senders.clone(),
            self.inner.pending_method_calls.clone(), already_read,
            #[cfg(unix)]
            already_received_fds,
            self.inner.socket_status.clone(),
        )
    }
