...
use std::collections::HashMap;
use zbus::{names::OwnedUniqueName, proxy::CacheProperties};
...
impl<'a> SecretService<'a> {
    ...
    /// Route a new instance to an already selected unique bus name.
    ///
    /// `connection` should be connected to the session/user message bus. This
    /// method neither resolves a well-known name nor creates another connection.
    /// Collection, item and prompt proxies inherit this service destination.
    ///
    /// This is routing only: the caller must monitor ownership changes. Session
    /// establishment and high-level reply decoding remain legacy and unbounded;
    /// this does not qualify a bounded connection, durable provider or persistent
    /// vault. The private checked-reply helpers are not integrated by this method.
    pub async fn connect_with_existing_unique(
        encryption: EncryptionType,
    ...
    pub async fn create_collection(
        ...
        let collection_path: ObjectPath = {
            ...
            if created_path.as_str() == "/" {
                ...
                let prompt_res = exec_prompt(&self.service_proxy, &prompt_path).await?;
                prompt_res.try_into()?
            } else {
    ...
    pub async fn unlock_all(&self, items: &[&Item<'_>]) -> Result<(), Error> {
        ...
        if lock_action_res.object_paths.is_empty() {
            exec_prompt(&self.service_proxy, &lock_action_res.prompt).await?;
        }
