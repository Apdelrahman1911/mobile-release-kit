...
pub(crate) async fn lock_or_unlock(
    ...
    if lock_action_res.object_paths.is_empty() {
        exec_prompt(service_proxy, &lock_action_res.prompt).await?;
    }
    Ok(())
}
...
// Routing follows the service proxy; the legacy prompt lifecycle and unbounded
// completion decoding remain unchanged and are not owner-reconciled here.
pub(crate) async fn exec_prompt(
    service_proxy: &ServiceProxy<'_>,
    prompt: &ObjectPath<'_>,
) -> Result<zvariant::OwnedValue, Error> {
    let service = service_proxy.inner();
    let prompt_proxy = PromptProxy::builder(service.connection())
