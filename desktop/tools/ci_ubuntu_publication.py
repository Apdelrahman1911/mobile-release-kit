...
def installed_shell_candidate(work, sha, *, lifecycle=None, local_transport=None, deadline=None):
...
def shell_source_manifest(source):
    ...
    paths = ("desktop/src-tauri/Cargo.toml", "desktop/src-tauri/Cargo.lock", "desktop/src-tauri/build.rs",
             ...
             "desktop/tools/ci_ubuntu_publication.py", "desktop/tools/ci_foundation.py", "desktop/tools/ubuntu_publication_lifecycle.py",
             "desktop/tools/prepare_hosted_ubuntu_data.py", "desktop/tools/android_material_preparation.py",
...
def verify_installed_shell():
    ...
    try:
        ...
        binaries, compiler, native, roster_sha, producer_attempt, artifact_id = installed_shell_candidate(
            work, sha, lifecycle=lifecycle, local_transport=transport, deadline=check.end)
        D.need(elf_dependencies(D.read(Path(library["path"]), MAX_BINARY)) == old_compiler["nativeInputs"]["outputs"]["libtest"]["elf"],
        ...
        shell = {"binaries": binaries, "compiler": projection, "rosterSha256": roster_sha,
                 ...
                 "androidPublication": lifecycle.shell_android_publication_request(root)}
        if same_job:
        ...
        source_record = {"sourceSha": sha, "sourceTree": compiler["sourceTree"], "runId": os.environ["GITHUB_RUN_ID"],
            ...
            "toolsInputPreparation": tools_inputs, "androidBuildPublication": compiler["androidBuildPublication"]}
        D.write(public / "source.json", D.canonical(source_record))
        request = {"sourceSha": sha, "runId": os.environ["GITHUB_RUN_ID"], "attempt": os.environ["GITHUB_RUN_ATTEMPT"],
            "deadline": deadline, "runnerUid": os.getuid(), "runnerGid": os.getgid(), "source": str(source), "taskRoot": str(root),
        ...
        print("Normal window, twenty-two success-requiring observers and one raw-exit1 expected negative retained with service finality; no product/package qualification.", flush=True)
    except BaseException as error:
