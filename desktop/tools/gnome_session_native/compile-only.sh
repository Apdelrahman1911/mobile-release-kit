...
/usr/bin/python3.12 -I -S -B -X pycache_prefix=/run/mrk-gnome-python-empty-pycache-v1 /prepare.py prepare
...
for phase in libtest; do
    ...
    (
        ...
        exec /usr/bin/timeout --signal=TERM --kill-after=5s 30s \
            /usr/bin/python3.12 -I -S -B -X pycache_prefix=/run/mrk-gnome-python-empty-pycache-v1 /check-compile.py "$phase"
    ) > "$receipt"
