...
# Preserve the exact existing twenty cases, including raw failure and saved version.
...
SHELL_PUBLIC_FILE_LIMIT = 185  # Exact24-case root roster:183, exported:185; non-shell remains128.
...
def shell_handoff(value, original_paths):
    ...
    need(type(shell) is dict and set(shell) == {"binaries", "compiler", "rosterSha256", "producerAttempt",
         "acceptedU", "loaderPolicy", "androidPublication"} | ({"localTransport"} if "localTransport" in shell else {"artifactId"}),
...
def _capacity(value):
    ...
    if "shell" in value:
        ...
        # bus-config files, its log, and a bus socket. The shell-only185 output
        # slots cover the183 root originals; TOTAL_LIMIT is unchanged.
...
def _domain(value, label):
    ...
    show = SHOW + (("LimitNOFILE", "LimitNOFILESoft") if "shell" in value else ())
...
def _shell_fixtures_prepare(value):
    """Create the23 fixed fixture trees once, retained on every failure.
...
def _finish_body(value, request_sha, start, states, observations, traces, cases, loader):
    ...
    if "installed" in value:
        ...
        android_publication = _finish_android_publication(value)
        ...
        extra = {"shellRosterSha256": shell["rosterSha256"], "shellProducerAttempt": shell["producerAttempt"],
                 **shell_transport_provenance(shell), "consumerAttempt": value["attempt"], "acceptedU": shell["acceptedU"],
                 "packageLifecycleQualified": False, "shellPackageBuilt": False, "androidPublication": android_publication}
