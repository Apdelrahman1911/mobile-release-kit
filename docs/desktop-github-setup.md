...
fixed caller files, when its own native qualification permits it. **That new
writer's production gate starts disabled**, independently of configuration
editing. Source implementation or inert renderer checks are not qualification.
The production runtime remains incomplete/disabled; no standalone desktop
readiness or native installer qualification is claimed.
...
Linux x86_64 GNU strong native registration is the first target. The distinct
workflow writer gate remains false until its exact source/ZIP direct-child and
original-owner filesystem/fault/STOP/settlement evidence is independently
reviewed. Configuration fixtures, pure UI tests, compilation, browser previews
and other-platform smoke evidence cannot enable it. Other-platform writes,
including Windows writes, remain unsupported by this slice.
