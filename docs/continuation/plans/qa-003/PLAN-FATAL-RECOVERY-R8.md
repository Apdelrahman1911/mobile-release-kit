# QA-003 R8 — preserve fatal cleanup across recovery and signing owners

## Status, finding and exact scope

PROPOSED; no production implementation changes authorized yet. This corrects
...
- In `cleanup_signing`, retain fatal errors from the active body and each cleanup
  owner rather than only a boolean. Already-registered independent native/profile
  cleanup is still attempted when existing dispatch/journal/retention predicates
  prove it safe; post-dispatch/uncontained sessions remain quarantined. Do not
  replay uncertain work. ALWAYS close owned raw session descriptors.
...
- Reconcile outer cancellation wrapping so a fatal failure remains structured
  even when a secondary expected error masks it. Do not change QA-004's distinct
  outer materialization scope or falsely claim that issue resolved.
...
- Handler restoration/cancellation combined with another cleanup failure: exact
  ...
  ordinary cancellation; pure cancellation retains130. All test-only handler
...
4. Supersede unused R2 preparation explicitly. Identity-check its unused owned
   temp parent/child/marker before cleaning only those disposable resources. Keep
