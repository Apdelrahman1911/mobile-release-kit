# QA-003 R8-R3 — preserve fatal cleanup across recovery and signing owners

## Status, finding and exact scope

PROPOSED R3; supersedes retained R8/R8-R2 proposals after independent review.
...
- At ENTRY to `cleanup_signing`, capture `exc_info()[1]` before any catch or
  cleanup. Retain the original entry exception AND a cumulative typed LIFETIME
  observation initialized with all its explicit facts, including NONFATAL
  ProcessError and ProcessInterrupted dispatch/containment. Explicitly merge EVERY
  later caught failure, even while the aggregate is not fatal. ExitStack.close()
  can detach callback exceptions from the active body; final-chain traversal or
  storing only fatal exceptions is therefore insufficient. If a later close turns
  the aggregate fatal, earlier dispatched/uncontained facts must still be present.
  Use the existing OR-dispatched/AND-contained/AND-cleanup_complete combiner; no
  text guessing. Raise only a fatal aggregate as fatal, not an ordinary completed
  error merely because it has dispatch history. For a fatal body/entry result
...
- Once a fatal failure is known, skip extra profile reconciliation and terminal
  ...
  error cannot replace the fatal result. Lease/session close aggregators likewise
  capture the active exception before their first catch and retain every observed
  typed fact; an earlier nonfatal/dispatched body must not be forgotten when an
  independent close later fails. Ordinary conflict cleanup still returns
...
- Both pre-effect and after-effect close failures for relevant local/installer
  ...
  handles close. Assert phase-specific status: a retained lease FD keeps flock
  busy until exact owner exit/closure; selector/profile failures may leave pending
  after lease release; terminal close failure can leave absent but must still
  error in the failed invocation. Never manufacture a pending marker for tests.
  For the intentionally retained FD, assert identity/liveness before
...
- Typed chained-error classifier tests, including ordinary controls, cycles,
  ...
  context, and multiple independent cleanup failures. Add a production composed
  signing regression with a NONFATAL dispatched ProcessError (and contained
  ProcessInterrupted) from the body, followed by a real profile FD-close failure
  through ExitStack.close where its exception context is actually detached. Verify
  that the final fatal error retains the earlier dispatch facts despite that
  detachment and all later caught failures. Do not mistake group absence
