# QA-003 R9 revision 4 — bind profile observations to the files used and removed

## Finding, scope and reanalysis

PROPOSED REVISION 4; no runtime implementation is authorized until independent
plan approval. Supersedes initial R9/R9-R2/R9-R3, retained unchanged. R3's
plan-only approval is ON HOLD after the additional actual composing-owner defect
below; it does not authorize implementation. This R4 addresses R9-PR-01/-02,
installer-borrowed full-stat clarity and preservation across BOTH cleanup owners.
...
- Root also reproduced a composing-owner defect on the UNMODIFIED runtime in
  reanalysis-profile-r9-r3/reproduce-outer-repeat.py/RESULT.json: a real same-inode
  chmod/mtime change during the actual bounded installer read raises the reader's
  genuine CredentialError. Installer cleanup preserves it, but outer automatic
  session.cleanup_profile immediately re-observes stable new metadata, deletes
  the changed inode and removes controls. Final signing still raises an ordinary
  CredentialError (NOT success); profile is gone/status idle. Both owners run;
  only native/CMS responses are fictional. Exact scratch/FDs were cleaned and no
  runtime/AGENTS edits occurred. Coherent snapshots alone cannot fix this, because
  persisted identity/hash intentionally contains no historical stat metadata.
...
4. Borrowed cleanup keeps the initial identity short-circuit (do not open a known
   ...
   to the recorded borrowed observation, then recheck current stat. Explicitly
   compare the full initial borrowed pathname stat with the returned read stat,
   as for owned cleanup: a same-inode metadata change in that interval remains a
   preserved conflict even if final bytes/identity match. No later fresh outer
   read may erase this observed installer failure. Any mismatch is a preserved
   cleanup conflict, never resolved success.
...
### B7. Remember profile conflicts across the composing lifetime

A failed observation must not be silently replaced by a later fresh observation
in the same implicit setup/teardown sequence. Add a narrow private optional
`on_conflict` callback to the installer and an invocation-local set of conflicted
filenames. This is NOT a new journal field, native quarantine policy or permission.

- Record the conflicted name and failed-cleanup status BEFORE invoking the callback.
  Coherent-reader failures, already-observed identity/content/metadata contradictions,
  unsafe conditional cleanup and ambiguous ordinary stat/unlink failures retain
  that name. Do not read or unlink it again in subsequent installer unwind.
- Preserve safe independent cleanup of OTHER known-original resources: e.g. an
  owned stage alias may be removed after destination content/metadata conflict,
  including partial writes. All raw handles are attempted once in finally. A
  conflict is not a blanket retention flag. Keep direct fatal snapshot_failed
  behavior stronger: no subsequent resource work/resolution after that failed
  read lifetime; R8 whole-graph fatal normalization remains unchanged.
- Initial/owned safe absence is not an observation failure or ownership grant;
  do not poison ordinary absent-file cleanup. Stable existing DIFFERENT content
  is a normal fail-closed admission rejection, not an invented change during
  inspection. Stable identical borrowed, hardlinks, partial stage writes and
  successful complete cleanup remain supported controls. Borrowed disappearance
  during cleanup is still a conflict because a borrowed file must be preserved.
- The composing `_temporary_apple_signing_environment` creates a `profile_conflict`
  boolean BEFORE installer `enter_context`. Pass a no-I/O callback that only
  latches True. This covers early failed entry before ExitStack callback registration
  as well as registered teardown. Do not infer profile ownership from error text,
  rely only on catching installed_profile.close, or collapse it into native
  session.unresolved/retain_profile (which would block independently safe cleanup).
- Run already-registered native cleanup, installer.close and session/raw close
  under their unchanged guards. AFTER those attempts, any latched profile conflict
  suppresses the owner's extra session.cleanup_profile/recover/finish and reports
  failure. No fresh snapshot can erase that prior contradiction or finalize the
  original controls in this failed signing invocation. No secret/Store work added.
- Keep the ORIGINAL intent/checkpoint on disk for explicit recovery after this
  invocation ends/quiesces. Fresh original-session recovery may re-observe stable
  original inode/hash; foreign destination is preserved/CLI1, unknown stage stays
  pending, changed bytes remain refused. Metadata is ephemeral, not historic
  durable policy. No guessed deletion, regenerated intent or timestamp authority.
- Compatibility change: some profile-conflict exits that previously immediately
  became idle now remain pending until explicit recovery. Revise existing foreign/
  missing borrowed and owned-replacement tests to REQUIRE pending and preservation
  BEFORE recovery, then actual CLI/fresh-process recovery and correct conflict
  semantics. Do not merely loosen status assertions or skip these tests. Native-
  preference-only conflicts retain their prior automatic safe cleanup semantics.

This no-implicit-retry latch is scoped to the composing signing lifetime. An
EXPLICIT locked/manual recovery `recheck` is a separate owner-confirmed observation
attempt, not an implicit retry; preserve its ordinary-conflict API. Fatal cleanup
still prevents all manual retry as required by R8. Clarify existing wording below
accordingly; do not accidentally reject legitimate owner-confirmed recovery.
...
- Original identity with changed bytes or metadata is not deletable in that
  observation attempt. Preserve it and fail with controlled CredentialError;
  ordinary filesystem/read errors keep remaining recovery state. Fresh explicit
  recovery, or the existing explicit locked/manual recheck after owner quiescence,
  can re-observe original identity/hash. No implicit retry may erase a failure.
...
- Full composing owner: genuine current-reader-detected mid-read chmod/mtime
  change, new stat→read and post-close mismatches, BOTH borrowed/owned, setup
  before ExitStack registration and registered normal teardown. Also post-stage
  final-admission mismatch and one-shot stat/unlink failures: no implicit outer
  reread/deletion after conflict, no terminal marker, pending original controls,
  independent native/stage/raw cleanup completed where safe. Verify preservation
  BEFORE fixture cleanup, then explicit fresh-process recovery from those controls.
  Keep native-preference-only conflict and stable initial mismatch/absence controls.
- Explicit locked manual recheck after an ordinary observed metadata conflict must
  remain distinct from automatic repeated reconciliation; no prompt/retry for a
  fatal read/close failure. Record actual observation attempts and exact user input.
...
- Stable original-owned, stable identical borrowed (including hardlinked files),
  ...
  read on BOTH recovery/normal cleanup and for BOTH owned/borrowed roles,
  post-read chmod, changed content, stable
...
profile conflicts remaining pending until explicit recovery instead of automatic
in-owner re-reconciliation, and explicit remaining non-cooperating host limits. Do not advertise an atomic
