# QA-003 R9 — bind profile observations to the files used and removed

## Finding, scope and reanalysis

PROPOSED; no runtime implementation is authorized until independent plan approval.
...
- Safe controls: borrowed recovery already preserves disappearance/replacement
  as a conflict; full borrowed cleanup detects it via the outer owner; ordinary
  installer absence safely cleans. Preserve these semantics.
...
In `SigningSession.cleanup_profile`, preserve the borrowed branch's conflict
semantics and original persisted authority. For each original-owned stage and
destination, consume the full `_read_regular` `(bytes, stat)` observation (or an
equivalent internal helper retaining it), not only a digest projection:
...
- Exercise normal composed signing teardown AND actual CLI recovery, owned
  ...
  reportable without deleting unrelated state.
...
- Stable original-owned, stable identical borrowed, ordinary disappearance,
  in-place changed content/metadata, partial-write stage cleanup and same/different
  UUID controls. Verify no retries on ambiguous descriptor close, independent
