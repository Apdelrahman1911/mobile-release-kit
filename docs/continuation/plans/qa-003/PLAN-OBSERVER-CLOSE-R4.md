# QA-003 R4 correction plan — PROPOSED, not implementation approval

Baseline main2beb373/tree00f3acee; entire implementation r1 still unchanged and
under independent whole-diff review. Read with approved R2/final R3. This closes
...
2. Outer installer cleanup must always relinquish BOTH still-owned raw descriptor
   ...
   recursive retry of a claimed close.
...
- Combined close uncertainty: close may fail before effect OR close successfully
  ...
  exact fallback only after evidence. Retain all existing close/fork/default-signal
