# QA-006 — verifier correction plan R4-R2
...
This complete revision preserves rejected proposal R4 (SHA256
`adaaf1d6cec4461a589bc4a4bc398000066c3367d38544618a418fcf89dc8f9e`)
and addresses independent PR4-01/02/03 in `verifier-plan-review-r4/PLAN-REVIEW.md`
(SHA256 `8ef8c899a21ad62e448af264d15429b6f395b156d180d693356842649cde5b17`),
decision SHA256 `d46a46c3628f5696bdd17a9c269686a93ee5d04cf6f9a01deb03ed81e33715a4`.
The corrections below are mandatory, not optional implementation suggestions.
R4-R2 needs its own exact independent approval before any helper modification.
...
- `CONFIG.json`, a new exact `DISCOVERY-ACCEPTANCE.json` static record, and all
  helper/static-policy bindings: explicit policy `qa006-verifier-r4-r2-v1`.
  Old snapshots are preserved, not silently migrated or accepted as this version.
  The discovery record binds initial controls/classification to independent
  verifier review; it is not self-approved by a fresh freeze or collection.
...
The separate primary signal-fence experiment ran once and now has independent
**PASS_RECORDED_PRIMARY_ONLY** reconciliation (decision SHA256
`2951408891c3f1a92d2f5524cbf91f716e322b4059b59e802d468c34628c030c`).
Its32 rows do not authorize rerun, inherited-chain experiments or full suites.
...
1. Top-level prepare/check/cleanup/controller-binding loads this authority once.
   ...
   Reject arbitrary dictionaries, a different ctx/root, and closed/expired/failed
   contexts. The factory registers the exact validated instance to this active
   context and canonical root; a lookalike class instance is not that authority.
...
6. Keep internal values deeply immutable (including nested mappings/sequences and
   references); never expose mutable aliases. Existing JSON/public-dict consumers
   may receive defensive copies, but such copies are never accepted as an
   authority bundle. Mutating an exported value cannot mutate trusted internals,
   become an accepted replacement, or change a resulting accepted record.
   Keep stable equality/canonical serialization on explicit validated values;
   do not use serialization work as an excuse to share a mutable authority alias.
7. Add an explicit Action lifecycle: ordinary work requires active ownership by
   the current `with action` invocation. Revoke authority in `finally` on normal,
   failed, cancelled and signal-handler-restoration exits, even before the time
   cutoff. Closed contexts cannot be re-entered, registered or used again.
   Revocation is distinct from expiry and clears ordinary authority registration.
8. Revocation must not prevent mandatory descriptor/child disposition or bounded
   FAIL-only diagnostics for an actual latched failure. Those paths retain the
   original cutoff/counters and concrete resource ownership; they cannot perform
   ordinary reads/writes, acquire workers, seal success, delete/adopt outputs or
   reopen the action. Internal terminal cancellation/deadline checks still run
   after handler restoration without pretending the action is newly active.
9. New actual actions always perform full original validation. This is a
   trusted-helper data-flow/lifetime contract, not secret-token security against
   arbitrary same-process Python introspection. No global/persistent cache or
   on-disk PASS flag is introduced.
...
### Stable source universe and separate fresh mirror evidence

Add exact stable fields `sourceUniverse` and `preservedUserUniverse`, one for
each canonical configured root, plus a stable binding of the independently
reviewed `DISCOVERY-ACCEPTANCE.json`. The complete per-root schema is:

- integer `schemaVersion=1`, canonical `root`, complete `rootIdentity`,
  exact `policyVersion`, and the bound fixed classification policy digest;
- `entries`: canonical non-excluded relative names mapped to exact
  `{kind, identity}` records. Identity contains the same typed
  device/inode/UID/mode/type fields as current no-follow source bindings.
  Every ordinary leaf and directory has identity/mode, not just a name/kind.
  Do not bind mutable directory timestamps or counts of pruned children;
- `controls`: every permitted effective `.gitignore` relative path mapped to
  exact `{file, tracked}`, with complete descriptor-read bytes/hash/identity
  binding and a strict boolean derived from the complete raw tracked index;
- `classification`: the fixed source/generated/private/structural-ancestor
  rules and execution-versus-preservation role. Its schema and policy digest
  must match the separately reviewed static policy, not native ignore output.

The ordinary selected-file payload bindings remain in the existing physical/
logical fields. Ordinary unknown/unselected payloads are never read; their
metadata can be recorded for a rejection, but they cannot enter accepted stable
authority merely because a fresh collection found them.

Require complete typed schemas and both universe fields in `validate_stable`,
canonical frozen equality and pre-publication/deletion checks. A fresh view
observation binds its universe to this exact stable value/digest. It separately
contains the actual newly owned mirror path/inode/nonce/inventory, real native
records and cleanup result. These latter values are volatile: two genuine
unchanged collections must have equal stable authority and different valid
owned mirror observations. Never strip stable identities to get equality.

### Independent initial discovery authority

`DISCOVERY-ACCEPTANCE.json` is a new private static input, included in the exact
helper/static inventory reviewed and bound by `ACCEPTED-VERIFIER`. It names
policy version and both canonical roots, and for each root binds:

1. the unchanged original independently reviewed logical source snapshot by its
   full reference/hash; selected names/bytes must still match that snapshot;
2. all permitted structural directories (required ancestors of accepted files,
   additional bound controls and AGENTS, plus any explicitly reviewed empty
   directory); unknown ordinary directory entries cannot approve themselves;
3. each additional non-selected ignore control by canonical relative path,
   byte length, SHA256, mode and expected raw-index tracked status;
4. the separately permitted AGENTS path/hash or explicit absence, and the exact
   execution-exclusion policy described below.

Controls already present in the accepted source snapshot must match its hash/
mode and raw-index-derived tracked state. Additional ignored/self-excluded
controls require the explicit new independently reviewed mapping; a freshly
produced freeze/mirror or the control's own ignore patterns cannot approve it.
For this actual snapshot the mapping is expected to contain no extra control;
measure and review it, never invent a prior approval. Historical acceptance/
snapshot bytes and schemas remain unchanged.

Before any package/test/code-consuming launch, allow outside fixed exclusions
only accepted source paths, permitted structural directories, independently
bound controls and separately bound AGENTS. Reject an ordinary unknown/unselected
entry or unapproved control before reading its payload. Native selected-name
equality alone is insufficient. A newly self-excluding control cannot hide an
unreviewed module from this gate. Synthetic positive self-exclusion cases use
new, explicitly rebound fixture authority; they do not claim historical approval.
...
The implementation must list exact root-relative, component and suffix semantics
rather than applying a broad substring test. Root `.git` is only the configured,
separately raw-bound locator/storage; an unknown nested `.git` entry in an
inspected source subtree is rejected without reading or traversing that storage.
Do not turn a nested repository boundary into an ordinary placeholder directory.
Fixed generated-region ancestors such as root `vendor` for `vendor/bundle` and
root `release` for `release/private` need explicit classification: accepting a
generated child does not authorize arbitrary sibling files or prune the whole
ancestor when it also contains accepted source.
...
classification is bound instead, and existing per-output ownership gates remain mandatory. Root AGENTS remains
separately byte-bound and never a deliverable file.

### Exclusion is not execution permission

For the **execution source**, check excluded input-sensitive regions before the
initial freeze and each applicable gate/subcommand that can consume them:

- Source bytecode/cache paths (`__pycache__`, `*.py[cod]`), local Bundler
  configuration/vendor paths, source-local venvs, private credential/signing/
  native-asset/evidence paths and unsupported nested Git boundaries must be
  absent unless a separately enumerated reviewed same-invocation producer rule
  explicitly applies. `-B` alone does not stop Python reading existing bytecode.
- This revision does not add a general exception mechanism. For this source,
  existing build-owner consumers may authorize only their exact
  `build/` and `src/mobile_release_kit.egg-info/` outputs, with their original
  owner/creation records, exact inventory and successful actual preceding
  producer completion. Wheel installations/staging remain in the separately
  owned outside-source root. Extend these exact guards/callers, not the scope of
  what a partial producer may adopt.
- Within a compound gate, an earlier actual subcommand exit/validated inventory
  may authorize its declared output; the still-running gate's future PASS may
  not. Before importing an installed wheel, preserve current complete source/
  wheel payload equality and additionally prove no source input was excluded
  from the independently accepted source set.
- Non-code generated data such as `.DS_Store` is classified as unread and
  non-consumable, not a general ignored-data import source. Other generated
  outputs lacking a known reviewed consumer remain non-consumable and need
  exact ownership before removal; no unknown `.bundle/config`, bytecode or
  credential file is opened merely to reject its existence.
- Unknown/symlinked execution-exclusion entries fail with retained state and
  zero later code-consuming launch. Wrong owner, extra contents, failed/partial
  producer or replacement cannot authorize reuse or deletion.

For the **preservation-only user root**, preserve its fixed excluded private/
generated regions without reading their contents. This does not grant permission
to run application/package gates there. Separately frozen verifier helpers/base
tool inputs under that root retain their existing explicit authority; the root's
ordinary ignored files do not inherit it. The future complete native launch
contract must also bind clean configuration/environment; this plan does not
broaden that launch authority or inspect/delete user configuration.

### Safe native mirror and final comparison

1. Create the mirror inside the current exclusively owned metadata view. Recreate
   only independently permitted inventoried directory names, empty regular
   placeholders with matching source executable/file-versus-directory semantics,
   and the exact
...
2. Bind the complete fresh mirror inventory and its derivation from the stable
   source-universe/control projection into the view observation. Native `ls-files` receives the existing copied raw index and
...
3. Mirror placeholders preserve file-versus-directory semantics; native Git remains
   the authority for actual Gitignore matching, not independent input approval. Verify stages/flags as before.
   ...
   read. Independently permitted non-selected controls are separately bound, not
   silently added to selected source. Missing/deleted tracked entries retain their
...
5. Before `collect` publishes, and again after later native work before prepare/check
   ...
   control inventory and compare it with the initial independently authorized
   stable record; compare execution-exclusion authorization where applicable. Also retain
...
| PR4-01 | Two genuine unchanged collect/check lifecycles have equal stable universes and different valid mirrors. Independently authorized self-excluded-control drift between actions fails actual `check_frozen` even with unchanged selected names/bytes. Test late ignored entries, modes and same-name inode replacement with no ordinary unselected/outside reads. |
| PR4-02 | New self-excluding control plus ignored module rejects before payload read or code gate. Explicitly bound control-only case succeeds. Re-freezing cannot adopt unapproved control bytes. Cover every tracked-in-exclusion class, exact ancestor/suffix semantics, nested Git boundary and unknown/symlinked bytecode/Bundler/private inputs. A real successful owned producer's exact output succeeds; wrong owner/extra content/failed producer/substitution reject with no consumption/deletion. |
| PR4-03 | Reuse genuine successful-but-closed still-before-cutoff ctx/bundle, second ctx/root, copied/lookalike instance, mutated exported nested value and changed backing reference: none can change accepted authority or enable work/publication/removal. Genuine same-action reuse avoids duplicate JSON without omitting checks. Restoration failure/cancellation still disposes owned resources and writes only original-budget failure evidence. |
...
This changes only unreleased private verifier contracts. Use explicit policy
`qa006-verifier-r4-r2-v1` and a new preparation namespace; rejected R3 records remain readable historical
...
1. Obtain independent approval of this exact revised plan and PR4-01/02/03
   corrections; revise before implementation if any initial input authority,
   stable/volatile universe, data-flow/lifetime, schema, resource or ownership
   requirement is unsound. No approval of the rejected R4 is carried forward.
