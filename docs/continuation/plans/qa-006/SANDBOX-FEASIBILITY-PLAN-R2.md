# QA-006 — sole-owner macOS signal-fence feasibility plan R2
...
Handlers for INT/TERM are installed before acquisition and only set a flag.
There is a handlers-default interval during exec; termination there kills the
sole sender rather than leaving a second dispatcher. The outer invocation fails
and children receive EOF. After re-entry, restore the same flag-only policy.
Cancellation/any failure disables all new signal dispatch, closes known control
writers, and joins only actual directly owned children. Keep initial error,
reap/EOF evidence and cleanup failures separate. No rescue may declare success
after timeout, cancellation, missing/changed records or ambiguous ownership.
...
7. **Lifecycle controls.** Cancel before signal dispatch and after readiness;
   inject malformed readiness and an early actual canary exit. Require zero
