...
## R2 — accurate result protocols

The standard-library-only Supply/WIF script is not Minitest. Classify explicit
gate result types: Python unittest, Ruby Minitest, Supply/WIF assertion script,
and ordinary command. Require the correct nonempty zero-error/zero-skip terminal
summary for Python/Minitest and the actual expected terminal
`locked Fastlane Supply WIF contract: PASS` after an exit0 for WIF. Do not count
a nested/printed unrelated summary as the current gate's result. Record the native
profile runner's actual nonempty/no-skip count as well. Include parser fixtures
for genuine summaries, skips, missing/empty/contradictory summaries and WIF
success/failure. This fixes a preparation-only protocol mismatch; it neither
weakens the Store-facing WIF contract nor adds a runtime dependency.
