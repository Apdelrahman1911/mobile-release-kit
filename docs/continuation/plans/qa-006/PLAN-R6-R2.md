# QA-006 R6-R2 — preserve unexpected native-fixture errors by execution state
...
For the pre-entry faults, every acquired worker must genuinely exit through its
original private EOF and be joined; check actual status and ECHILD/death, all acquired descriptors closed,
registered watchdog/injector joined, traps restored, no pending repeat, and zero
historical-PID fallback. No pre-entry capture/first-close false positive is allowed. The explicitly
entered substitution control below instead requires real native SIGKILL, not EOF. Retain StandardError versus runtime-redacted IOError distinctions.
...
## R6 independent review corrections — mandatory, not optional

Original R6 review `plan-review-r6/PLAN-REVIEW.md`, SHA256
`5ebe07909922bf17ec90d4b6da9ec4d9a402e2e3d31a59dca869251518951af2`,
requested these two direct proofs. R6 is not an approval. Its original proposal
and all rejected results remain unchanged.

### R6-PLAN-01 — real entered close with distinct unexpected object

Add a fixed isolated proof using the ordinary `native-setup-io-error` driver.
At the real unique first `stdin.close` callback, after the original descriptor
closes and the actual first-close flags/count are recorded, substitute a distinct
unexpected IOError (same diagnostic as the intended IOError) BEFORE raising the
intended object. A unique inspected source-anchor TracePoint at the actual
`raise @injected_error` is suitable; do not manufacture flags or a capture result.
The actual native wrapper must return its unchanged exact redacted ContractError.
The nested frame and independent final-boundary observer must retain the distinct
unexpected original object; the raw native driver must fail, never pass.

Independently assert actual capture entry, exact first-close callsite/close/count,
intended-versus-substituted identity and saved message, waiter status/SIGKILL,
actual death and no control-EOF fallback. This proof deliberately uses native
cleanup; the pre-entry EOF requirements do not apply. It must execute only under
the separately approved disposable isolation/hosted verification, not unguarded
on the shared host. Any guarded development probe must veto unsafe/unknown or
post-reap requests and cannot count as an unmodified full-suite pass. Add a
negative-control isolated source copy deleting only the intended-primary identity
predicate: this exact entered substitution proof must fail its regression oracle.

### R6-PLAN-02 — nested original with no callback and zero backend acquisition

Add a fixed ordinary-mode pre-callback IOError proof AFTER the actual nested
frame is assigned and BEFORE any native Open3 acquisition is delegated. Use a
unique actual pre-spawn source anchor within the nested protected `begin`;
assert the frame reference exists at injection. Observe native backend entry
independently, and require zero backend acquisitions/callback entries, nil
`@callback_error`, the exact original in `frame.primary`, the existing normal
redacted native return, and the exact original at the final outer result boundary.
Run without secondary error and with one later actual owned-descriptor close
error; all four actually acquired control/event descriptors must close.

No uncertain spawn can become a no-child proof. The probe faults before the
native spawn-attempt increment/delegation, independently observes no native
backend call, and only then uses the existing no-native-child completion facts.
Do not introduce generic unknown-ownership acceptance or invent a waiter/PID/status.
The native driver raw failed result remains distinct from the regression proof.

### Exact test integration and ownership

Use fixed allowlisted probe modes mapped to a small test-only subclass/wrapper;
run the underlying NativeSetupDriver in its ordinary mode, not an error-policy
mode. The matrix adds at least29 cases:24 pre-entry combinations, late teardown,
pre-entry lookalike, entered distinct-IOError, and two frame-only IOErrors.
Actual final-error identity is independently observed using one unique source
anchor (derive its line from the inspected file, never a stale hard-coded number).
Save original message and SystemExit status at injection creation so assertions
cannot become same-object/same-current-property tautologies. Register/join real
Thread#raise injectors under the existing deferred mask.

Preserve the original raw `result.json` semantics/error and nonzero driver status
in every failed-driver proof. If a separate proof envelope is used, it cannot
replace/mutate that raw result or conceal a cleanup failure. The parent has only
its direct unreaped driver authority; reported native identities still authorize
read-only death observation, never a cleanup signal. No additional implementation
path outside the two R6 files is authorized by these tests.
