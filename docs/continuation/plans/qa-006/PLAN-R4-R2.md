...
## R4-R1 — required independent review refinements

**PROPOSED pending re-review.** These details refine/override the corresponding
R4 text above; all other prior contracts and six-path tests-only scope remain.

### One negative-observation cutoff, not one renewed timeout per call

Preallocate an invocation-local native-death budget in run. Lazily establish its
ONE monotonic cutoff at the first native-death observation; share that exact
cutoff across leader/child identities AND the active-body/ensure path. If the
active observation exhausted it, ensure must not acquire a fresh five seconds.
Likewise retain one native-observation cutoff for each OwnershipProbe case's
independent fallback, shared by its markers and any cleanup-finally entry.

Implement explicit deadline-aware stopped polling: check the cutoff BEFORE each
observation, cap the /bin/ps capture budget at min(remaining, existing2seconds),
and check time again before accepting a result. Persistent indeterminate state
never resets the cutoff. The underlying capture child's bounded ownership-stop
cleanup remains a SEPARATE existing5second safety allowance on capture failure;
do not claim the entire nested stop must fit inside an already-expired observation
budget. Such a failure is still non-success and does not authorize another stop.

The driver's existing descendant-alive oracle remains a separate2second deadline
owned by that isolated driver; use an explicit stopped predicate with that fixed
cutoff rather than switching to the parent's5second default. Initial readiness
has its original fixed cutoff as well: deny indeterminate within the existing
readiness loop, cap observations by remaining time and never restart readiness.
No whole-gate retries or timeout increases. Ensure may independently finish safe
cleanup after an earlier parser failure but can never convert that original fatal
failure into success; it still shares the original negative-observation cutoff.

In the full real-run persistent-indeterminate regression, record actual monotonic
observation-call times and cleanup phase, prove no new observation phase/budget
starts after cutoff in ensure, require non-success/retained scratch, then restore
real observation and separately prove death before exact scratch removal. This
exercises native identities and active/ensure composition, not just a unit polling
loop or a mocked native capture. Injected observation results add no signal right.

### Explicit supported modifier grammar

Do not retain [A-Za-z+<>-]*. Recognize the constrained Linux/macOS union of supported
modifiers, in emission order with each optional at most once:

    [<N]? X? E? V? L? s? l? \+?

Spaces above separate grammar elements, not actual output. This covers nice flags,
Darwin traced/exiting/parent-wait/locked flags, session leader, Linux multithreaded
and foreground indicators. Darwin adv_cmds-237 print.c:481–495 corroborates its
subset; Linux ps stat documents <,N,L,s,l,+. No arbitrary letters, repeated flags,
question-mark suffixes, > or - are accepted. Primary ? is a specifically recognized
Darwin unknown state, not a suffix wildcard. A Z row with E is contradictory and
rejected (Darwin's emitter excludes E for zombies). Primary X stays indeterminate,
not a new definite-stopped shortcut. Unsupported output fails closed.

Add exact cases Zgarbage/Sgarbage/ZEvil, repeated/out-of-order/unknown modifiers,
Z?/?E?, foreign ? identity, and contradictory Z+E. Include supported modifier
combinations and existing192-row replay so strictness cannot silently break real
Mac observations. These are parser tests, not PID queries/signals.

### Setup-first plus cleanup failure and queued repeats

The isolated setup probe matrix covers all THREE APIs, TWO initial error types
(IOError and a deliberately synchronous non-StandardError such as Interrupt),
and TWO secondary-cleanup modes:

1. distinct synchronous cleanup failure;
2. a real self-SIGINT plus a distinct Thread#raise repeat from a joined injector
   during protected cleanup (with the setup exception already first).

That is12 cases per adapter context. Record exact setup/cleanup entry and actual
signal/injector delivery, assert ORIGINAL object AND message after returning to
normal caller operations, zero attempted native spawns, no pending test interrupts,
joined raiser, and exact restored handlers/inactive scope. Preserve cleanup warning
failures as secondary errors; record the observation without overwriting the first.
Do not substitute older active-first repeated-cancellation tests for this new
ordering. Retain/delete only each no-native-acquisition probe's own completed
scratch after the result and preservation proof are recorded. All existing real
spawn/reap/asynchronous/OS/hard-termination tests remain required.

## R4-R2 — compose the setup primary, cleanup fault AND queued repeats

**PROPOSED pending final plan recheck.** In the R4-R1 matrix, secondary mode2
queues the real self-SIGINT and joined asynchronous repeat DURING protected
cleanup, then ALSO raises the distinct synchronous cleanup fault before returning
from that cleanup boundary. The original setup object/message must survive all
three secondary events and later normal caller operations. Mode1 is the plain
cleanup-fault control. Counts stay3APIs×2setup types×2modes=12 per adapter context.
This composes cleanup reporting with pending-queue draining rather than testing
cancellation in lieu of the failing cleanup. All other R4-R1 requirements remain.
