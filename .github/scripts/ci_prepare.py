...
def prepare_inputs(*, source_root: Path, destination: Path, platform: str, deadline: float,
    ...
    """
    remaining(deadline)
    if scope not in {"platform", "signing-matrix", "signing-adapter", "store-lane"}:
    ...
    for name in ("python", *(("gems",) if scope in {"platform", "store-lane"} else ()),
    ...
    for asset in gems if scope in {"platform", "store-lane"} else ():
    ...
    result = {"schema": 1, "platform": platform, "python": "python", "gems": "gems" if scope in {"platform", "store-lane"} else None,
              "bundler": "gems/" + manifest["bundler"]["filename"] if scope in {"platform", "store-lane"} else None, "actionlint": actionlint,
